...
pub fn bpann_f32(row: &[f64], scale_x: bool, x_scale: &[f64], out: &mut Vec<f32>) {
    out.clear();
    if scale_x {
        out.extend(
            row.iter()
...
pub fn sq_f32(query: &[f32], rows: &[Vec<f32>]) -> Vec<f32> {
    rows.iter().map(|r| l2_f32(query, r)).collect()
}
...
pub fn sq_rows(
    ...
    for &id in row_ids {
        let row = train_x.row_slice(id as usize)?;
        out.push(sq_l2(
...
mod tests {
    ...
    fn distance_agrees() {
        let mut buf = Vec::new();
        crate::distance::bpann_f32(&[1.0, 2.0], false, &[1.0, 1.0], &mut buf);
        assert_eq!(buf, vec![1.0, 2.0]);
        assert!((crate::distance::l2_f32(&[0.0, 0.0], &[1.0, 0.0]) - 1.0).abs() < 1e-6);
        let a = array![0.0, 0.0];
        let b = array![1.0, 0.0];
        let s = array![1.0, 1.0];
        let _ = crate::distance::sq_l2(a.view(), b.view(), false, s.view());
        let dir = TempDir::new().unwrap();
        let mut store = MmapColumnStore::open_mmap(dir.path().join("x.bin"), 2, None).unwrap();
        store
            .mmap_append(&array![[0.0, 0.0], [1.0, 0.0]].view())
            .unwrap();
        let _ = crate::distance::sq_f32(&[0.0, 0.0], &[vec![1.0, 0.0]]);
        let _ = crate::distance::sq_rows(&[0.0, 0.0], &store, &[0, 1], false, &[1.0, 1.0]).unwrap();
    }
}
