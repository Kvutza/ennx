...
pub(crate) fn first_mmap(ctx: &IndexBuildContext<'_>, row: usize) -> Result<Vec<f32>, BpannError> {
    let raw = ctx.train_x.row_slice(row)?;
    let mut out = Vec::with_capacity(ctx.num_dim);
    bpann_f32(raw, ctx.scale_x, ctx.x_scale, &mut out);
    Ok(out)
}

pub(crate) fn mean_f32(vectors: &[Vec<f32>]) -> Vec<f32> {
    if vectors.is_empty() {
        return Vec::new();
    }
...
pub(crate) fn load_mmap(
    ...
    for i in start..end {
        let row = ctx.train_x.row_slice(i)?;
        bpann_f32(row, ctx.scale_x, ctx.x_scale, &mut vec_buf);
        vectors.push(std::mem::take(&mut vec_buf));
    }
    Ok(vectors)
}

pub(crate) fn centroid_rows(
    ctx: &IndexBuildContext<'_>,
    start: usize,
    end: usize,
...
pub(crate) fn range_index(
    ...
    BpannIndex::pages_unpersisted(ctx.num_dim, end - start, leaf_rows, pages, index_dir)
}
...
pub(crate) fn vector_index(
    ...
    while cur < end {
        ...
        let vectors = load_mmap(ctx, cur, chunk_end)?;
        let centroid = mean_f32(&vectors);
        child_page_ids.push(next_id);
        child_centroids.push(centroid.clone());
...
mod tests {
    ...
    fn ctx_rows(n: usize) -> (TempDir, MmapColumnStore, [f64; 2], Vec<f64>) {
    ...
    #[test]
    fn forest_paths() {
        let (dir, store, scale, _) = ctx_rows(300);
        ...
        let c0 = first_mmap(&ctx, 0).unwrap();
        assert_eq!(c0.len(), 2);
        let mean = mean_f32(&[vec![1.0, 2.0], vec![3.0, 4.0]]);
        assert!((mean[0] - 2.0).abs() < 1e-6);
        assert!(mean_f32(&[]).is_empty());
        let vectors = load_mmap(&ctx, 0, 4).unwrap();
        assert_eq!(vectors.len(), 4);
        let cent = centroid_rows(&ctx, 0, 10).unwrap();
        assert_eq!(cent.len(), 2);
        assert!(centroid_rows(&ctx, 5, 5).unwrap().is_empty());

        let empty = range_index(&ctx, 0, 250, 64, dir.path().join("e")).unwrap();
        assert_eq!(empty.header.indexed_rows, 250);
        assert!(empty.pages.len() >= 3);
        let vect = vector_index(&ctx, 0, 200, 50, dir.path().join("v")).unwrap();
        ...
        // Scaled path for centroid_rows / first_row
        ...
        let _ = first_mmap(&ctx_s, 1).unwrap();
        let _ = centroid_rows(&ctx_s, 0, 3).unwrap();
        let _ = load_mmap(&ctx_s, 0, 2).unwrap();
    }

    #[test]
    fn empty_n() {
        let (dir, store, scale, _) = ctx_rows(4096);
        let ctx = IndexBuildContext {
            train_x: &store,
            num_dim: 2,
            scale_x: false,
        ...
        let index = range_index(&ctx, 0, 4096, 64, dir.path().join("e")).unwrap();
    ...
    fn range_leaf() {
        ...
        let start = 16u32;
        let end = 80u32;
        let range_page = Page::empty_leaf(1, start, end, vec![0.0, 0.0]);
        ...
        let a = score_page(Some(&mmap_store), &query, r_ids, r_range, r_vecs).unwrap();
        let b = score_page(Some(&mmap_store), &query, i_ids, i_range, i_vecs).unwrap();
        assert_eq!(a, b);
