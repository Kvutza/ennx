...
pub fn kmeans_init(points: &[Vec<f32>], k: usize, rng: &mut impl Rng) -> Vec<Vec<f32>> {
    ...
    while centroids.len() < k {
        ...
        for p in points {
            let d = centroids
                .iter()
                .map(|c| crate::distance::l2_f32(p, c))
    ...
    points
        ...
        .map(|p| {
            centroids
                ...
                .enumerate()
                .min_by(|(_, a), (_, b)| {
                    crate::distance::l2_f32(p, a)
                        .partial_cmp(&crate::distance::l2_f32(p, b))
...
pub fn kmeans_run(
    ...
    let mut centroids = kmeans_init(points, k, rng);
...
fn partition_recursive(
    ...
    partition_vectors(row_ids, vectors, leaf_capacity, rng)
}
...
fn partition_vectors(
    ...
    let children: Vec<PartitionNode> = clusters
        ...
        .map(|(c, vecs)| {
            ...
            partition_vectors(&c, &sub_vectors, leaf_capacity, rng)
        })
        .collect();
...
pub fn max_size(node: &PartitionNode) -> usize {
    match node {
        PartitionNode::Leaf { entries, .. } => entries.len(),
        PartitionNode::Internal { children, .. } => {
            children.iter().map(max_size).max().unwrap_or(0)
        }
    }
}
...
mod tests {
    ...
    fn kmeans_capacity() {
        ...
        assert!(max_size(&tree.root) <= leaf_capacity);
    }
}
