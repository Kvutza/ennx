...
use crate::distance::{bpann_f32, l2_f32, sq_f32};
...
pub fn score_rows(
    ...
    if !vectors.is_empty() {
        let dists = sq_f32(query, vectors);
        return Ok(row_ids
            .iter()
            .zip(dists.iter())
    ...
    for &row_id in row_ids {
        let row = store.train_x.row_slice(row_id as usize)?;
        bpann_f32(row, store.scale_x, store.x_scale, &mut vec_buf);
        scored.push((row_id, l2_f32(query, &vec_buf)));
    }
    Ok(scored)
}
...
pub(crate) fn score_page(
    ...
    if let Some((start, end)) = row_range {
        ...
        return score_range(store, query, start, end);
    }
    score_rows(store, query, row_ids, vectors)
}

pub const MAX_LEAVES: usize = 384;

pub fn search_leaves(index: &BpannIndex, query: &[f32], k: usize) -> Vec<(u32, f32)> {
    search_store(index, query, k, None).expect("search_leaves")
}
...
pub fn search_store(
    ...
    for page in &index.pages {
        ...
        {
            scored.extend(score_page(store, query, row_ids, *row_range, vectors)?);
        }
    }
    scored.sort_by(|a, b| {
        a.1.partial_cmp(&b.1)
...
pub fn search_index(
    ...
    if let Some(root) = index.page(root_id) {
        collect_leaves(
            root,
            query,
            index,
    ...
    if use_skip_edges {
        ...
        for leaf_id in initial {
            if candidate_leaves.len() >= MAX_LEAVES {
                break;
            }
    ...
    truncate_leaves(index, query, &mut candidate_leaves, MAX_LEAVES);
...
fn truncate_leaves(index: &BpannIndex, query: &[f32], leaves: &mut VecDeque<u32>, max: usize) {
    if leaves.len() <= max {
        return;
    }
    let mut ranked: Vec<(u32, f32)> = leaves
        ...
        .filter_map(|&leaf_id| {
            index
                .page(leaf_id)
                .map(|page| (leaf_id, l2_f32(query, &page.centroid())))
        })
        .collect();
    ...
    match page {
        ...
        Page::Internal {
            ...
            for (i, _) in ranked.into_iter().take(beam_width.max(1)) {
                ...
                if let Some(child) = index.page(child_id) {
                    collect_leaves(child, query, index, beam_width, visited, queue, visited_log);
                }
            }
        }
    }
...
pub fn search_only(
    ...
    search_tree(index, query, k, beam_width, None).expect("search_only")
}
...
pub fn search_refinement(
    ...
    refine_store(index, query, k, beam_width, visited_log, None).expect("search_refinement")
}
...
pub fn bpann_k(vectors: &[Vec<f32>], queries: &[Vec<f32>], k: usize, index: &BpannIndex) -> f64 {
    if queries.is_empty() {
        return 0.0;
    }
    let mut total = 0.0;
    for q in queries {
        let bf = bpann_topk(vectors, q, k);
        let bf_set: HashSet<u32> = bf.iter().map(|(id, _)| *id).collect();
        let approx = search_leaves(index, q, k);
...
pub fn bpann_topk(vectors: &[Vec<f32>], query: &[f32], k: usize) -> Vec<(u32, f32)> {
    let mut scored: Vec<(u32, f32)> = vectors
        .iter()
        .enumerate()
        .map(|(i, v)| (i as u32, l2_f32(query, v)))
        .collect();
    scored.sort_by(|a, b| {
        a.1.partial_cmp(&b.1)
            .unwrap_or(Ordering::Equal)
...
pub fn bpann_mmap(
    ...
    let mut vec_buf = Vec::new();
    for i in start..end {
        let row = train_x.row_slice(i)?;
        crate::distance::bpann_f32(row, scale_x, x_scale, &mut vec_buf);
        let mut acc = 0.0f32;
        if scale_x {
...
mod tests {
    ...
    fn search_returns() {
        ...
        let index =
            BpannIndex::build_vectors(&vectors, 2, LEAF_CAPACITY, 0, dir.path().join("index"))
                .unwrap();
        let _ = crate::index::search::bpann_k(&vectors, &[vec![0.0, 0.0]], 1, &index);
        let _ = crate::index::search::bpann_topk(&vectors, &[0.0, 0.0], 1);
        let mut store =
            crate::mmap_store::MmapColumnStore::open_mmap(dir.path().join("x.bin"), 2, None)
                .unwrap();
        store
        ...
        let _ = crate::index::search::bpann_mmap(&store, 0, 2, &[0.0, 0.0], 1, false, &x_scale)
            .unwrap();
        let _ = crate::index::search::search_only(&index, &[0.0, 0.0], 1, 2);
        let mut log = Vec::new();
        let _ = crate::index::search::search_refinement(&index, &[0.0, 0.0], 1, 2, &mut log);
    }
}
