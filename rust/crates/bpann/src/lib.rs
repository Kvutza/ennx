...
mod acceptance_tests {
    ...
    #[test]
    fn test_001() {
        let err = backend::open_dim(MAX_DIM + 1).unwrap_err();
        assert!(
            err.to_string().contains("MAX_DIM") || err.to_string().contains(&MAX_DIM.to_string())
        );
    }

    #[test]
    fn test_002() {
        let huge_dim = MAX_STRIDE / 8 + 1;
        let err = backend::open_stride(huge_dim).unwrap_err();
    ...
    fn test_008() {
        ...
        .unwrap();
        let (tx, ty, yv) = b.train_rows(&[1]).unwrap();
    ...
    fn brute_oracle(b: &BpannBackend, query: &[f64], k: usize) -> Vec<i64> {
        let mut scored: Vec<(i64, f64)> = (0..b.len())
            .map(|i| {
                let row = b.row_slice(i).unwrap();
                let mut acc = 0.0;
    ...
    fn test_010() {
        ...
        let (d, idx) = b
            ...
            .unwrap();
        let expected = brute_oracle(&b, q.as_slice().unwrap(), 10);
        let got: Vec<i64> = idx.row(0).iter().copied().collect();
        assert_eq!(got, expected);
        for j in 0..10 {
            let row = b.row_slice(expected[j] as usize).unwrap();
            let mut acc = 0.0;
    ...
    #[test]
    fn test_cp0bpannramrecall() {
        let vectors = synthetic_f32(N, D, DATA_SEED);
        let queries = synthetic_f32(NUM_QUERIES, D, QUERY_SEED);
        let dir = TempDir::new().unwrap();
        let index = BpannIndex::build_vectors(
            &vectors,
            D,
            crate::index::LEAF_CAPACITY,
            0,
            dir.path().join("index"),
        )
        .unwrap();
        let recall = bpann_k(&vectors, &queries, K, &index);
    ...
    fn test_016() {
        ...
        let (d, idx) = b
            ...
            .unwrap();
        let neighbor = b.row_slice(idx[[0, 0]] as usize).unwrap();
        let expected = sq_l2(q.view(), neighbor.into(), true, scale.view());
    ...
    fn test_019() {
        ...
        // when bpann_mmap double-applies x_scale, so k=1 leg_b drops row 2.
        b.append_rows(
    ...
    fn test_024() {
        ...
        assert_eq!(b.soft_threshold(), PENDING_SOFT);
        assert_eq!(PENDING_SOFT, 250);
        assert_eq!(crate::BpannTuning::default().soft_threshold, PENDING_SOFT);
    }
    ...
    fn test_025() {
        let dir = TempDir::new().unwrap();
        let b = BpannBackend::new_empty(dir.path().to_path_buf(), 2, 1).unwrap();
        assert_eq!(b.hard_threshold(), PENDING_HARD);
    ...
    fn test_026() {
        ...
        let b = BpannBackend::new_empty(dir.path().to_path_buf(), 2, 1)
            .unwrap()
            .with_soft(2000);
        assert_eq!(b.soft_threshold(), 2000);
        assert!(b.hard_threshold() >= 2000);
    }
    ...
    fn test_scalecheckpointsci() {
        ...
        let (_, i_out) = b
            ...
            .unwrap();
        let expected = brute_oracle(&b, q.as_slice().unwrap(), 10.min(n));
        let got: Vec<i64> = i_out.row(0).iter().copied().collect();
        assert_eq!(got, expected);
    }

    #[test]
    #[ignore = "Run manually: cargo test -p bpann test_scalerecallignored -- --ignored --nocapture"]
    fn test_scalerecallignored() {
        let n = 1000usize;
        let d = 32usize;
        let dir = TempDir::new().unwrap();
        let (x, y) = synthetic_train(n, d, 42);
        ...
        let vectors: Vec<Vec<f32>> = (0..b.len())
            .map(|i| b.row_slice(i).unwrap().iter().map(|&v| v as f32).collect())
            .collect();
        let queries: Vec<Vec<f32>> = (0..3)
            .map(|i| x.row(i).iter().map(|&v| v as f32).collect())
            .collect();
        let index = b.index_snapshot().unwrap();
        let recall = bpann_k(&vectors, &queries, 10, index);
        assert!(recall >= 0.90, "N={n} recall={recall}");
    }

    #[test]
    #[ignore = "Run manually: cargo test -p bpann test_scale10mignored -- --ignored --nocapture"]
    fn test_scale10mignored() {
