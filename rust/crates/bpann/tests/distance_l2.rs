...
//! `l2_f32`, which was rewritten to use lane accumulators for SIMD.
//!
...
fn l2_properties() {
    let seed: u64 = rand::random();
    println!("l2_properties seed={seed}");
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    for _ in 0..500 {
        ...
        assert_eq!(l2_f32(&a, &a), 0.0, "self-distance nonzero seed={seed}");
        ...
        let scaled = l2_f32(&a_scale, &b_scale);
