...
use bpann::distance::{bpann_f32, l2_f32, sq_l2};
...
fn sq_f643(a: &[f64], b: &[f64], scale_x: Option<&[f64]>) -> f64 {
    match scale_x {
        Some(scale) => a
            .iter()
            .zip(b.iter())
...
fn finite_f32(rng: &mut ChaCha8Rng, len: usize, scale: f32) -> Vec<f32> {
    (0..len).map(|_| rng.gen_range(-scale..=scale)).collect()
}

fn finite_f64(rng: &mut ChaCha8Rng, len: usize, scale: f64) -> Vec<f64> {
    (0..len).map(|_| rng.gen_range(-scale..=scale)).collect()
}

fn close_reduction(candidate: f32, reference: f64, len: usize) -> bool {
    ...
    for &seed in CORPUS_SEEDS {
        ...
        for &len in ADVERSARIAL_LENGTHS {
            for case in 0..32 {
                ...
                let a = finite_f32(&mut rng, len, scale);
                let b = finite_f32(&mut rng, len, scale);
                let got = l2_f32(&a, &b);
                let reversed = l2_f32(&b, &a);
                let reference = sq_f642(&a, &b);
                ...
                assert!(
                    close_reduction(got, reference, len),
...
fn sq_scaling() {
    for &seed in CORPUS_SEEDS {
        ...
        for len in [1usize, 2, 3, 8, 17, 64, 129] {
            for case in 0..24 {
                let a = finite_f64(&mut rng, len, 100.0);
                let b = finite_f64(&mut rng, len, 100.0);
                ...
                let unscaled_reference = sq_f643(&a, &b, None);
                let scaled_reference = sq_f643(&a, &b, Some(&scale));
...
fn bpann_l2() {
    for &seed in CORPUS_SEEDS {
        ...
        for len in [1usize, 4, 8, 17, 65] {
            for case in 0..24 {
                let row = finite_f64(&mut rng, len, 25.0);
                let query = finite_f64(&mut rng, len, 25.0);
                let scale: Vec<f64> = (0..len).map(|_| rng.gen_range(0.05f64..=20.0)).collect();

                let mut encoded_row = Vec::new();
                bpann_f32(&row, true, &scale, &mut encoded_row);
                ...
                let encoded_distance = l2_f32(&encoded_query, &encoded_row);
                let reference = sq_l2(
                ...
                assert!(
                    close_reduction(encoded_distance, reference, len),
