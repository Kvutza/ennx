...
pub fn y_py<'py>(py: Python<'py>, y: PyReadonlyArray1<f64>) -> PyResult<(f64, f64)> {
...
/// Python wrapper for pareto2d_max
#[pyfunction(name = "pareto2d_max")]
...
pub fn pareto_py<'py>(
    ...
    let result = py.detach(|| ennx::pareto2d_max(&a_arr, &b_arr, idx_vec.as_deref()));
...
/// Python wrapper for sobol_indices
...
pub fn sobol_py<'py>(
    ...
    let sobol = py.detach(|| ennx::sobol_indices(&x_arr, &y_arr));
...
/// Python wrapper for pareto_arms (returns selected candidate rows).
#[pyfunction(name = "pareto_arms")]
...
#[pyfunction(name = "quantize_e2m1")]
#[pyo3(signature = (arr, scale=1.0))]
pub fn bind_quantize<'py>(
    py: Python<'py>,
    arr: PyReadonlyArrayDyn<f32>,
    scale: f32,
) -> PyResult<Bound<'py, PyArray1<u8>>> {
    let values = arr.as_array();
    let packed = py.detach(|| ennx::experimental::quantize_e2m1(values.iter().copied(), scale));
    Ok(Array1::from_vec(packed).into_pyarray(py))
}

/// Override ennx config path (`None` restores `~/.ennx/config.toml`).
#[pyfunction(name = "set_path")]
#[pyo3(signature = (path=None))]
pub fn set_py(path: Option<&str>) -> PyResult<()> {
    ennx::set_path(path.map(std::path::PathBuf::from)).map_err(PyValueError::new_err)?;
    Ok(())
}

/// Ensure `~/.ennx/config.toml` (or override) exists and return its path.
#[pyfunction(name = "ensure_config_file")]
pub fn ensure_py() -> PyResult<String> {
    ennx::bpann_config().map_err(PyValueError::new_err)?;
