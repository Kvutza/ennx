...
pub(crate) fn apply_overrides(
    ...
    overrides.num_samples = optional_usize(dict, "num_samples")?;
    overrides.num_candidates = optional_usize(dict, "num_candidates")?;
...
fn parse_driver(s: &str) -> PyResult<ennx::index::IndexDriver> {
...
fn parse_rv(s: &str) -> PyResult<ennx::CandidateRV> {
...
fn enn_storage(s: &str) -> PyResult<ennx::EnnStorage> {
    match s.to_lowercase().as_str() {
...
pub fn parse_dict(dict: &Bound<'_, pyo3::types::PyDict>) -> PyResult<ennx::ConfigOverrides> {
    use ennx::ConfigOverrides;

    let mut overrides = ConfigOverrides::default();

    if let Some(v) = dict.get_item("index_driver")? {
        overrides.index_driver = Some(parse_driver(&v.extract::<String>()?)?);
    }
    ...
    if let Some(rv) = dict.get_item("candidate_rv")? {
        overrides.candidate_rv = Some(parse_rv(&rv.extract::<String>()?)?);
    }
    ...
    if let Some(v) = dict.get_item("rescalarize")? {
        ...
        let resc = match s.as_str() {
            "on_restart" => ennx::mbtrregn::Rescalarize::OnRestart,
            "on_propose" => ennx::mbtrregn::Rescalarize::OnPropose,
    ...
    if let Some(v) = dict.get_item("enn_storage")? {
        overrides.enn_storage = Some(enn_storage(&v.extract::<String>()?)?);
    }
    ...
    apply_overrides(dict, &mut overrides)?;
    Ok(overrides)
}
...
impl PyOptimizer {
    ...
    fn tell(
        ...
        let result = match seed {
            Some(seed) => {
                let mut rng = StdRng::seed_from_u64(seed);
                self.inner
                    .tell_variance(&x_unit.view(), &y_view, yvar_view.as_ref(), &mut rng)
            }
            None => {
                self.inner
                    .tell_variance(&x_unit.view(), &y_view, yvar_view.as_ref(), &mut self.rng)
            }
        }
        .map_err(|e| PyValueError::new_err(e.to_string()));
        if result.is_ok() {
    ...
    fn unit_incumbent<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyArrayDyn<f64>>> {
        self.inner
            .unit_incumbent()
...
fn build_optimizer(
    ...
    use ennx::optimizer_factory::{
        create_overrides, enn_overrides, lhd_overrides, region_overrides,
    };

    let num_dim = bounds.nrows();
    let overrides = cfg.map(parse_dict).transpose()?;
    let mut rng = StdRng::seed_from_u64(seed);
    let optimizer = match kind {
        ...
        "gp" => {
            ...
            let mut config = ennx::turbo_zero();
