...
fn k_16() {
    ...
    let metal = match KnnIndex::new(&train.view(), IndexDriver::Metal) {
        Ok(index) => index,
        Err(error) if metal_unavailable(&error.to_string()) => return,
...
fn wide() {
    ...
    let exact = KnnIndex::new(&train.view(), IndexDriver::Exact).unwrap();
    let metal = match KnnIndex::with_plan(&train.view(), IndexDriver::Metal, KnnPlan::Wide) {
        Ok(index) => index,
        Err(error) if metal_unavailable(&error.to_string()) => return,
        Err(error) => panic!("{error}"),
    };
...
fn tree() {
    ...
    let exact = KnnIndex::new(&train.view(), IndexDriver::Exact).unwrap();
    let metal = match KnnIndex::with_plan(&train.view(), IndexDriver::Metal, KnnPlan::Tree) {
        Ok(index) => index,
        Err(error) if metal_unavailable(&error.to_string()) => return,
        Err(error) => panic!("{error}"),
    };
...
fn gram() {
    ...
    let exact = KnnIndex::new(&train.view(), IndexDriver::Exact).unwrap();
    let metal = match KnnIndex::with_plan(&train.view(), IndexDriver::Metal, KnnPlan::Gram) {
        Ok(index) => index,
        Err(error) if metal_unavailable(&error.to_string()) => return,
        Err(error) => panic!("{error}"),
    };
