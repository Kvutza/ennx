...
const STREAMING_THRESHOLD: usize = 5;
const STREAMING_ROWS: usize = STREAMING_THRESHOLD + 1;
...
pub fn run_threshold(driver: IndexDriver) {
    ...
    let mut model = ENN::new_empty(
        ...
        Some(STREAMING_THRESHOLD),
    )
    .expect("new_empty disk");

    let mut row = 0usize;
    while row < STREAMING_ROWS {
        let end = (row + STREAMING_BATCH).min(STREAMING_ROWS);
        let (x, y) = random_batch(&mut rng, end - row, STREAMING_D);
        model.add(&x.view(), &y.view(), None).expect("add");
        row = end;
    }
    assert_eq!(model.len(), STREAMING_ROWS);
    model.index_access().ensure_sync().expect("sync");
    assert_eq!(model.len(), STREAMING_ROWS);
}

pub fn run_search(driver: IndexDriver) {
    let mut rng = ChaCha8Rng::seed_from_u64(STREAMING_SEED);
    let dir = TempDir::new().expect("tempdir");
    let mut model = ENN::new_empty(
        STREAMING_D,
    ...
    // This counter reports persisted index files, not the in-memory soft-sync pages.
    model.persist_index().expect("persist");
    assert_eq!(model.neighbors(&query.view(), 5, false).unwrap(), idx);
    let mem = model
        .index_access()
        .memory_bytes()
...
#[test]
fn appending_rows() {
    run_threshold(IndexDriver::BpAnnDisk);
}

#[test]
fn appended_sync() {
    run_search(IndexDriver::BpAnnDisk);
}
