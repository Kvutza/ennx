...
fn model_rows(n: usize, scale_x: bool) -> ENN {
    ENN::new(
        deterministic_x(n),
        deterministic_y(n),
        None,
...
fn model_yvar(n: usize, scale_x: bool) -> ENN {
    ENN::new(
        deterministic_x(n),
        deterministic_y(n),
...
fn timed_yvar(starting_rows: usize, num_adds: usize, scale_x: bool, with_yvar: bool) -> Duration {
    let mut model = if with_yvar {
        model_yvar(starting_rows, scale_x)
    } else {
        model_rows(starting_rows, scale_x)
    };
...
fn best_secs(starting_rows: usize, num_adds: usize, scale_x: bool, with_yvar: bool) -> f64 {
    (0..TIMING_REPEATS)
        .map(|_| timed_yvar(starting_rows, num_adds, scale_x, with_yvar).as_secs_f64())
        .min_by(|a, b| a.partial_cmp(b).expect("timing samples must be ordered"))
        .expect("at least one timing repeat")
}

fn measure_growth(scale_x: bool, with_yvar: bool) -> Vec<(usize, f64)> {
    ROW_COUNTS
        .into_iter()
        .map(|n| (n, best_secs(n, NUM_ADDS, scale_x, with_yvar)))
        .collect()
}

fn flat_violation(measurements: &[(usize, f64)], scale_x: bool, with_yvar: bool) -> Option<String> {
    ...
    let budget_secs = baseline_secs * MAX_SEC;
        ...
        if secs > budget_secs {
            return Some(format!(
                "single-row add at n={n} took {secs}s, budget {budget_secs}s \
                 ({MAX_SEC}x baseline {BASELINE_ROWS}={baseline_secs}s) \
                 scale_x={scale_x} with_yvar={with_yvar}: {measurements:?}",
            ));
        }
    }
    if observed_peak_ratio > MAX_SEC {
        return Some(format!(
            "peak ratio {observed_peak_ratio} exceeds calibrated cap {MAX_SEC} \
             scale_x={scale_x} with_yvar={with_yvar}: {measurements:?}",
        ));
    }
    None
...
fn assert_config(scale_x: bool, with_yvar: bool) {
    let mut last_measurements = Vec::new();
    for attempt in 0..MAX_ATTEMPTS {
        let measurements = measure_growth(scale_x, with_yvar);
        last_measurements.clone_from(&measurements);
        if let Some(message) = flat_violation(&measurements, scale_x, with_yvar) {
            assert!(attempt + 1 != MAX_ATTEMPTS, "{message}",);
            continue;
        }
        return;
    }
    panic!(
        "flat growth check failed after {MAX_ATTEMPTS} attempts \
...
#[test]
fn x_succeeds2() {
    let mut model = model_rows(100, true);
