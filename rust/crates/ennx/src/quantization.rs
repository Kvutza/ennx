...
pub fn quantize_e2m1(values: impl IntoIterator<Item = f32>, scale: f32) -> Vec<u8> {
    pack_nibbles(values.into_iter().map(|value| {
        let scaled = value / scale;
        let mut best_code = 0u8;
        let mut best_dist = f32::INFINITY;
        for (code, &candidate) in FP4_LUT.iter().enumerate() {
...
#[cfg(test)]
mod tests {
    use super::{quantize_e2m1, quantize_int4};

    #[test]
    fn pack_odd() {
        assert_eq!(quantize_int4([0.0, 1.0, 2.0], 1.0), vec![0x10, 0x02]);
        assert_eq!(quantize_e2m1([0.0, 1.0, 2.0], 1.0), vec![0x20, 0x04]);
    }
