...
impl MmapColumnStore {
    ...
    fn capacity_bytes(&self, nrows: usize) -> usize {
        nrows.saturating_mul(self.row_bytes())
    }

    fn ensure_capacity(&mut self, need_rows: usize) -> Result<(), ENNError> {
        let need_bytes = self.capacity_bytes(need_rows);
        if need_bytes <= self.mmap.len() {
            return Ok(());
        }
        let grow_rows = (need_rows - self.nrows).max(GROW_ROWS);
        let new_len = self.capacity_bytes(self.nrows + grow_rows);
    ...
    pub fn row_slice(&self, i: usize) -> Result<&[f64], ENNError> {
        if i >= self.nrows {
            return Err(ENNError::InvalidParameter(format!(
    ...
    pub(crate) fn mmap_range(&self, start: usize, end: usize) -> Result<Array2<f64>, ENNError> {
        if start > end {
            return Err(ENNError::InvalidParameter(format!(
                "mmap_range: start {start} > end {end}"
            )));
        }
        if end > self.nrows {
            return Err(ENNError::InvalidParameter(format!(
                "mmap_range end {end} out of range [0, {})",
                self.nrows
            )));
        }
        let n = end - start;
...
mod mmap_store {
    ...
    fn MmapColumnStore() {
        ...
        let store = MmapColumnStore::open_mmap(dir.path().join("c.bin"), 2, None).unwrap();
    ...
    fn capacity_bytes() {
        ...
        let dir = TempDir::new().expect("tempdir");
        let store = MmapColumnStore::open_mmap(dir.path().join("c.bin"), 2, None).unwrap();
        assert_eq!(store.capacity_bytes(4), 64);
    }
    ...
    fn ensure_capacity() {
        ...
        let dir = TempDir::new().expect("tempdir");
        let mut store = MmapColumnStore::open_mmap(dir.path().join("c.bin"), 2, None).unwrap();
    ...
    fn mmap_churn() {
        ...
        let mut store = MmapColumnStore::open_mmap(path, 2, None).unwrap();
        let n = 400usize;
        ...
        assert_eq!(store.row_slice(n - 1).unwrap()[0], (n - 1) as f64);
    }
    ...
    fn mmap_rows3() {
        ...
        let dir = TempDir::new().expect("tempdir");
        let mut store = MmapColumnStore::open_mmap(dir.path().join("c.bin"), 3, None).unwrap();
        ...
        assert_eq!(
            store.row_slice(0).unwrap(),
            &[1.0, 2.0, 3.0],
            "row0 corrupted under Fortran layout"
        );
        assert_eq!(store.row_slice(1).unwrap(), &[4.0, 5.0, 6.0]);
    }
    ...
    fn api_direct() {
        ...
        store
            .mmap_append(&array![[1.0, 2.0], [3.0, 4.0]].view())
            .unwrap();
        assert_eq!(store.row_slice(1).unwrap()[0], 3.0);
        let gathered = store.mmap_gather(&[0, 1]).unwrap();
        assert_eq!(gathered.nrows(), 2);
        assert_eq!(store.mmap_range(0, store.nrows).unwrap().nrows(), 2);
        let mid = store.mmap_range(1, 2).unwrap();
