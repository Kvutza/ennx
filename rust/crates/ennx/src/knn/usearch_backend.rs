...
mod binding {
    ...
    extern "C" {
        ...
        fn ennx_destroy(handle: *mut c_void);
        #[link_name = "ennx_usearch_reserve"]
        fn ennx_reserve(
            handle: *mut c_void,
            capacity: usize,
            error: *mut c_char,
        ...
        #[link_name = "ennx_usearch_expansion_search"]
        fn ennx_search2(handle: *const c_void) -> usize;
        #[link_name = "ennx_usearch_memory_usage"]
        fn ennx_usage(handle: *const c_void) -> usize;
    }

    fn error_buffer(buffer: &[c_char], fallback: &str) -> IndexError {
    ...
    fn call_bool(ok: bool, error: &[c_char], fallback: &str) -> Result<(), IndexError> {
        if ok {
            Ok(())
        } else {
            Err(error_buffer(error, fallback))
        }
    }
    ...
    impl Drop for BackendIndex {
        fn drop(&mut self) {
            if !self.handle.is_null() {
                unsafe { ennx_destroy(self.handle) };
    ...
    impl BackendIndex {
        pub(super) fn new(num_dim: usize) -> Result<Self, IndexError> {
            ...
            let handle = unsafe { ennx_new(num_dim, error.as_mut_ptr(), error.len()) };
            if handle.is_null() {
        ...
        pub(super) fn reserve(&mut self, capacity: usize) -> Result<(), IndexError> {
            let mut error = [0 as c_char; ERROR_LEN];
            let ok =
                unsafe { ennx_reserve(self.handle, capacity, error.as_mut_ptr(), error.len()) };
            call_bool(ok, &error, "failed to reserve native USearch capacity")
        }
        ...
        pub(super) fn add(&mut self, key: u64, vector: &[f32]) -> Result<(), IndexError> {
            ...
            call_bool(ok, &error, "failed to add a native USearch vector")
        }
        ...
        pub(super) fn search(
            ...
            call_bool(ok, &error, "native USearch search failed")?;
            keys.truncate(count);
            Ok(keys)
        }

        pub(super) fn expansion_search(&self) -> usize {
            unsafe { ennx_search2(self.handle) }
        }

        pub(super) fn memory_bytes(&self) -> usize {
            unsafe { ennx_usage(self.handle) }
        }
    }
}
...
impl USearchBackend {
    ...
    pub(crate) fn search(
        ...
        Ok(pad_k(distances, indices, search_k))
    }
}
