...
impl Optimizer {
    fn pick_incumbent(&mut self, candidate_indices: &[usize]) -> Result<(), ENNError> {
        ...
        self.unit_incumbent = Some(self.obs_access().x_row(best)?);
        self.y_scalar = Some(Array1::from_elem(1, best_mu));
        Ok(())
    }

    #[allow(dead_code)] // retained for tests / morbo callers; tell_turbo must not use it
    pub(crate) fn reset_tracker(&mut self) {
    ...
    pub fn update_incumbent(&mut self, rng: &mut dyn RngCore) -> Result<(), ENNError> {
        ...
        if self.config.noise_aware && self.surrogate.is_some() {
            return self.pick_incumbent(&candidate_indices);
        }
