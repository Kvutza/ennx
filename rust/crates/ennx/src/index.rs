...
pub fn disk_driver(driver: IndexDriver) -> bool {
...
mod tests {
    ...
    fn run_regression(make: fn(Array2<f64>) -> ENNIndex) {
        let n = 10usize;
    ...
    #[test]
    fn test_007() {
        run_regression(|train_x| index_unit(train_x, IndexDriver::Exact));
    }
    ...
    #[test]
    fn helpers_preserve() {
        use crate::knn::{flatten_f32, pad_k, unpack_search};
        let rows = array![[1.0, 2.0], [3.0, 4.0]];
        let f32v = flatten_f32(&rows.view());
        assert_eq!(f32v.len(), 4);
        let (d, _i) = pad_k(array![[1.0, 2.0]], array![[0i64, 1]], 3);
        assert_eq!(d.ncols(), 3);
        let (d2, i2) = unpack_search(1, 2, &[0.5f32, 1.5], &[0, 1]);
        assert_eq!(d2.shape(), &[1, 2]);
        assert_eq!(i2.shape(), &[1, 2]);
    }

    #[test]
    fn test_scaledsearch() {
    ...
    fn index_memory() {
        ...
        index
            .rebuild_scaled(array![[0.0, 0.0], [1.0, 1.0]], array![1.0, 1.0])
            .unwrap();
