...
impl InMemoryEnnBackend {
    pub fn new(
        ...
        Ok(Self {
            train_x_rows: RowStorage::from_array(train_x),
            train_y_rows: RowStorage::from_array(train_y),
            train_yvar_rows: train_yvar.map(RowStorage::from_array),
            num_dim,
            num_metrics,
            index,
    ...
    pub fn x_view(&self) -> ndarray::ArrayView2<'_, f64> {
        self.train_x_rows.view()
    }

    pub fn y_view(&self) -> ndarray::ArrayView2<'_, f64> {
    ...
    pub fn append_rows(
        ...
        match (&mut self.train_yvar_rows, yvar) {
            ...
            (None, Some(yv)) => {
                self.train_yvar_rows = Some(RowStorage::from_array(yv.to_owned()));
            }
    ...
    pub fn mark_stale(&self) {
        disk_obs::set_stale(&self.index_stale);
    }

    pub fn index_stale(&self) -> bool {
        disk_obs::read_stale(&self.index_stale)
    }
    ...
    pub fn ensure_sync(&self, scale_x: bool, x_scale: &Array1<f64>) -> Result<(), ENNError> {
        if scale_x {
            ...
            self.index.rebuild_scaled(train_x_scaled, x_scale.clone())?;
            *stale = false;
            *self
                .index_synced_obs
                .lock()
    ...
    pub fn train_rows(&self, indices: &[usize]) -> Result<super::TrainRows, ENNError> {
        let n = self.len();
        for &i in indices {
            if i >= n {
...
mod memory_unit {
    ...
    fn covers_search() {
        ...
        .unwrap();
        let (x, y, yv) = backend.train_rows(&[1]).unwrap();
        assert_eq!(x.nrows(), 1);
        assert_eq!(y.nrows(), 1);
        assert!(yv.is_none());
        ...
        backend
            ...
            .unwrap();
        assert!(backend.index_bytes().unwrap() > 0);
    }
}
