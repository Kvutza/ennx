...
pub enum AcquisitionKind {
    ...
    ///
    /// This is intentionally distinct from `crate::acquisition::ParetoAcquisition`,
    /// which performs multiobjective Pareto-front selection.
    Pareto,
}
...
impl ComputeDevice {
    pub fn parse(name: &str) -> Result<Self, String> {
        match name.trim().to_ascii_lowercase().as_str() {
            ...
            "cuda" => Ok(Self::Cuda),
            other => Err(format!(
                "unknown compute device {other:?}; expected 'auto', 'cpu', 'metal', 'opencl', or 'cuda'"
            )),
        }
    }
}
...
/// Score a single candidate in the materialized weight-selection path.
///
...
mod sparse;
pub use sparse::{
    apply_sparse, blocks_words, draw_sparse, merge_values, missing_words, sparse_union, sparse_xor,
    take_words,
};
#[cfg(test)]
