...
pub(crate) fn check_layout(leaves: &[Parameter]) -> Result<usize, String> {
    if leaves.is_empty() {
...
pub(crate) fn make_steps(leaves: &[Parameter], length: f32) -> Vec<LeafStep> {
...
pub(crate) fn make_tiles(leaves: &[Parameter]) -> Vec<Tile> {
...
mod tests {
    ...
    #[test]
    fn steps() {
        let leaves = [Parameter::new(0, 8, 4, 0.5, 1.0, 0.25).unwrap()];
