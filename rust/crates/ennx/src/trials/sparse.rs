...
pub(super) fn sparse_materialize(
    ...
    for &edit in edits {
        ...
        let step = super::make_steps(&[Parameter { offset: 0, ..leaf }], length)[0];
...
fn row_distance(base: &[u8], row: &[u8], leaves: &[Parameter]) -> f32 {
...
fn locate(global: usize, leaves: &[Parameter]) -> (usize, usize) {
...
fn global_index(edit: SparseEdit, leaves: &[Parameter]) -> usize {
...
fn byte_offset(leaves: &[Parameter], leaf: usize) -> usize {
...
mod tests {
    ...
    #[test]
    fn edits_unique() {
        let leaves = [Parameter::new(0, 17, 4, 0.25, 1.0, 0.25).unwrap()];
    ...
    #[test]
    fn changes_exact() {
        let leaves = [Parameter::new(0, 16, 4, 0.25, 1.0, 0.25).unwrap()];
        let base = [0x88; 8];
