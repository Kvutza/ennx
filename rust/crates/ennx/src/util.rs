...
/// Index of maximum value, choosing uniformly among ties (matches Python `argmax_tie`).
pub fn argmax_tie(values: &[f64], rng: &mut dyn rand::RngCore) -> usize {
    if values.is_empty() {
        return 0;
    }
...
pub fn sobol_indices(x: &ArrayView2<f64>, y: &ArrayView1<f64>) -> Array1<f64> {
    let n = x.nrows();
    let d = x.ncols();
...
pub fn pareto_arms(
    ...
    while !remaining.is_empty() && i_keep.len() < num_arms {
        let front = pareto2d_max(mu, se, Some(&remaining)).expect("mu and se must be finite");
        if front.is_empty() {
            break;
        }
...
mod tests {
    ...
    fn test_006() {
        ...
        let err = pareto2d_max(&a.view(), &b.view(), None).unwrap_err();
    ...
    fn test_007() {
        ...
        let front = pareto2d_max(&a.view(), &b.view(), Some(&[0, 2])).unwrap();

        assert_eq!(front, vec![0, 2]);
    }

    #[test]
    fn test_sobolindicesshape() {
        let x = array![[0.0, 0.0], [0.5, 0.5], [1.0, 1.0]];
        let y = array![0.0, 0.5, 1.0];

        let sobol = sobol_indices(&x.view(), &y.view());
    ...
    fn test_011() {
        ...
        let arms = pareto_arms(&x_cand.view(), &mu.view(), &se.view(), 5, 0);
    ...
    fn test_012() {
        ...
        let arms = pareto_arms(&x_cand.view(), &mu.view(), &se.view(), 2, 42);
        assert_eq!(arms.len(), 2);
        let arms2 = pareto_arms(&x_cand.view(), &mu.view(), &se.view(), 2, 42);
    ...
    fn test_013() {
        ...
        let idx = argmax_tie(&values, &mut rng);
        assert!(idx == 1 || idx == 2);
    }
