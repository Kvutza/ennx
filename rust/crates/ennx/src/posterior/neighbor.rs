...
pub(crate) fn dist2s_indices(model: &ENN, x: &ArrayView2<f64>, idx: &Array2<i64>) -> Array2<f64> {
    let n_query = idx.nrows();
    let k = idx.ncols();
    let mut out = Array2::zeros((n_query, k));
    if let Some(train_x) = model.x_opt() {
        for i in 0..n_query {
            let x_row = x.row(i);
            for j in 0..k {
                let ni = idx[[i, j]];
                if ni < 0 {
                    ...
                    out[[i, j]] = squared_l2(x_row, t_row, model.scale_x, model.x_scale.view());
                }
            }
        }
        return out;
    ...
    if !unique.is_empty() {
        let (train_x, _, _) = model
            .rows()
            .train_rows(&unique)
            .expect("train_rows for neighbors");
...
fn faiss_row(dist2s_faiss: &Array2<f64>, idx_faiss: &Array2<i64>, row: usize) -> Vec<(f64, i64)> {
...
#[allow(dead_code)]
fn batched_threshold(n_query: usize) -> usize {
    (n_query / 4).max(2)
}

#[allow(dead_code)]
pub(crate) fn use_batch(n_query: usize, escalate_count: usize, tie_neighbors: bool) -> bool {
    ...
    let _ = (n_query, escalate_count, tie_neighbors);
    false
}

pub(crate) fn f64_topk(
    model: &ENN,
    x: &ArrayView2<f64>,
    search_k: i32,
    ...
    let faiss_search_k = if tie_neighbors && k < n_train {
    ...
    if use_matrix_topk {
        ...
        let (train_x, _, _) = model.rows().train_rows(&all_indices).expect("train_rows");
        let dist_mat = distance_matrix(x, &train_x.view(), model.scale_x, &model.x_scale.view());
        ...
        for i in 0..n_query {
            ...
            let best = topk_buffers(row_slice, k, true, &mut topk_scratch, &mut float_buf);
            for (j, &t) in best.iter().enumerate() {
                dist2s[[i, j]] = row_slice[t];
                idx[[i, j]] = t as i64;
            }
        }
        return Ok(apply_nearest(dist2s, idx, exclude_nearest));
    }

    let (dist2s_search, idx_faiss) = model.backend_search(x, faiss_search_k as i32, false)?;
    let dist2s_faiss = if tie_neighbors {
        dist2s_indices(model, x, &idx_faiss)
    } else {
        dist2s_search
    };
    ...
    for i in 0..n_query {
        let mut pairs = faiss_row(&dist2s_faiss, &idx_faiss, i);
        let mut ctx = FaissPoolFinalizeCtx {
            precomputed_row_dists: None,
            faiss_pool_size: faiss_search_k,
            tie_scratch: &mut tie_scratch,
        };
        if finalize_topk(model, x.row(i), &mut pairs, k, tie_neighbors, &mut ctx) {
            let row_dists_vec = row_query(model, x.row(i));
            let best = topk_dists(&row_dists_vec, k, true);
            for (j, &t) in best.iter().enumerate() {
    ...
    Ok(apply_nearest(dist2s, idx, exclude_nearest))
}
...
pub(crate) fn get_data(
    ...
    let (dist2s_full, idx_full) =
        super::index_search(model, x, search_k as i32, exclude_nearest, tie_neighbors)?;
...
pub(crate) fn conditional_data(
    ...
    let search_k =
        (params.k_neighbors as usize + if flags.exclude_nearest { 1 } else { 0 }).min(total_n);
    ...
    let n_train = model.num_obs();
    let k = params.k_neighbors as usize;

    let train_search_k = search_k.min(n_train);
    let (dist2_train, idx_train) = if train_search_k > 0 {
        super::index_search(model, x, train_search_k as i32, false, flags.tie_neighbors)?
    } else {
        (
...
#[cfg(test)]
mod tests {
    use super::super::neighbor_dist::squared_l2;
    use super::{
        apply_nearest, conditional_data, dist2s_indices, distance_matrix, f64_topk, faiss_row,
    };
    ...
    fn faiss_threshold() {
        ...
        let pairs = faiss_row(&dist2s, &idx, 0);
        assert_eq!(pairs, vec![(0.0, 0), (1.0, 1)]);
        assert_eq!(batched_threshold(1024), 256);
        assert_eq!(batched_threshold(8), 2);
    }

    #[test]
    fn use_perf() {
        assert!(!use_batch(1024, 1024, true));
        assert!(!use_batch(1024, 300, false));
    }
    ...
    fn f64_count() {
        ...
        let (dist2s, idx) = f64_topk(&model, &train_x.view(), k as i32, false, true).unwrap();
        ...
        for i in [0usize, 16, 500, n - 1] {
            ...
            let mut ref_pairs: Vec<(f64, i64)> = (0..n)
                .map(|j| {
                    (
                        squared_l2(x_row, train_x.row(j), false, model.x_scale.view()),
                        j as i64,
                    )
                })
                .collect();
    ...
    fn f64_ties() {
        ...
        let model = ENN::new(train_x.clone(), train_y, None, false, IndexDriver::Exact).unwrap();
        let (dist2_batch, idx_batch) = f64_topk(&model, &train_x.view(), 10, false, true).unwrap();
        for i in 0..train_x.nrows() {
            let row = train_x.slice(ndarray::s![i..i + 1, ..]);
            let (dist2_one, idx_one) = f64_topk(&model, &row, 10, false, true).unwrap();
    ...
    fn f64_data() {
        ...
        let model = ENN::new(train_x.clone(), train_y, None, false, IndexDriver::Exact).unwrap();
        let k = 10usize;
        let (_dist2_on, idx_on) = f64_topk(&model, &train_x.view(), k as i32, false, true).unwrap();
        let _ = f64_topk(&model, &train_x.view(), k as i32, false, false).unwrap();
        for i in 0..train_x.nrows() {
            let x_row = train_x.row(i);
    ...
    fn f64_self() {
        ...
        let (dist2s, idx) = f64_topk(&model, &query.view(), 2, true, true).unwrap();
    ...
    fn apply_zeros() {
        let dist2s = array![[0.0, 1.0], [2.0, 3.0]];
        let idx = array![[0i64, 1], [1, 0]];
        let (d, i) = apply_nearest(dist2s.clone(), idx.clone(), true);
        assert_eq!(d.shape(), [2, 1]);
        assert_eq!(i[[0, 0]], 1);
        let (d0, i0) = apply_nearest(
        ...
        let (d2, i2) = apply_nearest(dist2s.clone(), idx.clone(), false);
        assert_eq!(d2, dist2s);
        assert_eq!(i2, idx);
    }
    ...
    fn f64_empty() {
        ...
        let model = ENN::new(train_x.clone(), train_y, None, false, IndexDriver::Exact).unwrap();
        let (dist2s, idx) = f64_topk(&model, &train_x.view(), 1, true, false).unwrap();
    ...
    fn f64_cutoff() {
        ...
        let model = ENN::new(train_x.clone(), train_y, None, false, IndexDriver::Exact).unwrap();
        let query = array![[0.0]];
        let (_, idx_on) = f64_topk(&model, &query.view(), 2, false, true).unwrap();
    ...
    fn sq_unscaled() {
        ...
        let d_unscaled = distance_matrix(&x.view(), &y.view(), false, &scale.view());
        assert_eq!(d_unscaled.nrows(), 2);
        assert_eq!(d_unscaled.ncols(), 2);
        let d_scaled = distance_matrix(&x.view(), &y.view(), true, &scale.view());
    ...
    fn sq_each() {
        ...
        let d = distance_matrix(&x.view(), &y.view(), false, &scale.view());
        assert_eq!(d.nrows(), 1);
        assert_eq!(d.ncols(), 1);
    ...
    fn dist2s_block() {
        ...
        let dist2s = dist2s_indices(&model, &query.view(), &idx);
        let block = distance_matrix(&query.view(), &train_x.view(), false, &array![1.0].view());
    ...
    fn sq_unscaled2() {
        ...
        let d0 = distance_matrix(&x.view(), &y.view(), false, &scale.view());
