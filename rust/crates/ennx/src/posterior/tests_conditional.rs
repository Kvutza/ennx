...
fn test_001() {
    ...
    let cond = conditional_internals(
        &model,
        &query.view(),
        &x_whatif.view(),
        &y_whatif.view(),
...
fn test_004() {
    let model = create_test_model();
    let params = ENNParams::new(2, 1.0, 0.1).unwrap();
    let flags = PosteriorFlags::new().exclude_nearest(true);
    let query = array![[0.5, 0.5]];
...
fn test_005() {
    ...
    let model = ENN::new(train_x, train_y, None, true, IndexDriver::Exact).unwrap();
    let params = ENNParams::new(2, 1.0, 0.1).unwrap();
    let flags = PosteriorFlags::new();
...
fn test_007() {
    ...
    let s0 = compute_conditional(&train.view(), &whatif_empty.view());
    assert_eq!(s0.len(), 1);
    assert!((s0[0] - 1.0).abs() < 1e-12);

    let train_c = array![[1.0, 2.0], [1.0, 3.0]];
    let whatif_c = array![[1.0, 0.0]];
    let s1 = compute_conditional(&train_c.view(), &whatif_c.view());
    assert_eq!(s1.len(), 2);
...
fn test_008() {
    ...
    let result = conditional_internals(
        &model,
        &query.view(),
        &x_whatif.view(),
        &y_whatif.view(),
...
fn test_013() {
    ...
    assert!(
        result.is_ok(),
        "compute_posterior with empty idx should not panic"
    );
}
