use ennx::experimental::{
    AcquisitionKind, ComputeDevice, EncodingType, SearchCenter, SearchConfig,
};
use ennx::search::Parameter;
use ennx::search::{Search, Trial};
...
fn run() -> Result<(), String> {
    test_basicasktell()?;
    test_lazyask()?;
    test_allacquisitions()?;
    test_allencodings()?;
    test_replacehistory()?;
    test_batchcandidates()?;
    test_multiregions()?;
    test_compactcentertree()?;
    println!("CUDA_TRIAL ok=true suites=8 target=sm_75");
    Ok(())
}

fn test_basicasktell() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 257, 4, 0.125, 1.0, 0.75)?,
        Parameter::new(257, 259, 8, 0.03125, 1.0, 0.5)?,
    ];
    let row_bytes = 257_usize.div_ceil(2) + 259;
    let base = make_base(row_bytes);
    let mut cpu = Search::new(&base, -0.75, leaves.clone(), 6, ComputeDevice::Cpu)?;
    let mut cuda = Search::new(&base, -0.75, leaves, 6, ComputeDevice::Cuda)?;
...
fn test_lazyask() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 128, 4, 0.25, 1.0, 0.8)?,
        Parameter::new(128, 128, 8, 0.05, 1.0, 0.6)?,
    ];
    let row_bytes = 128_usize.div_ceil(2) + 128;
    let base = make_base(row_bytes);
    let mut cpu = Search::new(&base, 0.0, leaves.clone(), 4, ComputeDevice::Cpu)?;
    let mut cuda = Search::new(&base, 0.0, leaves, 4, ComputeDevice::Cuda)?;
...
fn test_allacquisitions() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 64, 4, 0.125, 1.0, 0.5)?,
        Parameter::new(64, 64, 8, 0.05, 1.0, 0.5)?,
    ];
    ...
    for acq in [
        ...
        let mut cpu = Search::new(&base, 0.5, leaves.clone(), 4, ComputeDevice::Cpu)?;
        let mut cuda = Search::new(&base, 0.5, leaves.clone(), 4, ComputeDevice::Cuda)?;
        let config = SearchConfig {
            length: 0.6,
            neighbors: 1,
            acquisition: acq,
...
fn test_allencodings() -> Result<(), String> {
    let encodings = [
        (EncodingType::Int4, 4),
        (EncodingType::Int8, 8),
    ...
    for (encoding, bits) in encodings {
        let leaf = Parameter::encoded(0, 128, bits, encoding, 0.125, 1.0, 0.5)?;
        let row_bytes = if bits == 4 { 64 } else { 128 };
        let base = make_base(row_bytes);
        let mut cpu = Search::new(&base, 1.0, vec![leaf], 4, ComputeDevice::Cpu)?;
        let mut cuda = Search::new(&base, 1.0, vec![leaf], 4, ComputeDevice::Cuda)?;
...
fn test_replacehistory() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 64, 4, 0.25, 1.0, 0.75)?,
        Parameter::new(64, 64, 8, 0.5, 0.5, 1.0)?,
    ];
    let row_bytes = 64_usize.div_ceil(2) + 64;
    let base = make_base(row_bytes);
    let mut cpu = Search::new(&base, 0.0, leaves.clone(), 4, ComputeDevice::Cpu)?;
...
fn test_batchcandidates() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 512, 4, 0.125, 1.0, 0.75)?,
        Parameter::new(512, 512, 8, 0.03125, 1.0, 0.5)?,
    ];
    let row_bytes = 512_usize.div_ceil(2) + 512;
    let base = make_base(row_bytes);
    let mut cpu = Search::new(&base, 0.0, leaves.clone(), 8, ComputeDevice::Cpu)?;
    let mut cuda = Search::new(&base, 0.0, leaves, 8, ComputeDevice::Cuda)?;
...
fn test_multiregions() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 257, 4, 0.125, 1.0, 0.75)?,
        Parameter::new(257, 259, 8, 0.03125, 1.0, 0.5)?,
    ];
    let row_bytes = 257_usize.div_ceil(2) + 259;
    let base = make_base(row_bytes);
    let mut cpu = Search::new(&base, 0.25, leaves.clone(), 4, ComputeDevice::Cpu)?;
    let mut cuda = Search::new(&base, 0.25, leaves, 4, ComputeDevice::Cuda)?;
    ...
    let expected = cpu.ask_regions(3, 3, &seeds, config)?;
    let actual = cuda.ask_regions(3, 3, &seeds, config)?;
    compare_regions(&expected, &actual)
}

fn test_compactcentertree() -> Result<(), String> {
    let leaves = vec![
        Parameter::new(0, 257, 4, 0.125, 1.0, 0.75)?,
        Parameter::new(257, 259, 8, 0.03125, 1.0, 0.5)?,
    ];
    ...
    let expected = cpu.ask_centers(2, 3, &centers, &region_centers, &seeds, config)?;
    let actual = cuda.ask_centers(2, 3, &centers, &region_centers, &seeds, config)?;
    compare_regions(&expected, &actual)
}
...
fn compare_trial(cpu: Trial, cuda: Trial) -> Result<(), String> {
