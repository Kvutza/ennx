...
def weighted_case(train, queries, values) -> None:
    ...
    cuda = ENN(train, values, variances, False, "cuda")
    cuda.set_neighbors(False)
    ...
    seeds = [7, -3, 101]
    draws, draw_indices = cuda.posterior_draw(
        queries,
        6,
        0.7,
        0.13,
    ...
    cond_draws, cond_indices = cuda.conditional_draw(
        whatif_x,
        whatif_y,
        queries,
        6,
...
def knn_case(rows: int, dims: int) -> None:
    ...
    cuda = ENN(train, values, None, False, "cuda")
    cuda.set_neighbors(False)
