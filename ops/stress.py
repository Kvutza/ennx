...
def rss_bytes2(*, num_dim: int, shard_max_rows: int = DEFAULT_SHARD_MAX_ROWS) -> int:
...
def rss_stress(
    *,
    ...
    x_query = make_points(cfg.query_n, num_dim=cfg.num_dim, seed=STRESS_QUERY_SEED)
...
def parse_driver2(name: str) -> ENNIndexDriver:
    mapping = {
...
def enn_chunk(index_driver: ENNIndexDriver) -> int:
...
def make_points(query_n: int, *, num_dim: int, seed: int) -> np.ndarray:
...
def function_sample(seed: int, num_seeds: int = 1) -> list[int]:
    if num_seeds < 1:
        raise ValueError("num_seeds must be >= 1")
    return list(range(seed, seed + num_seeds))


def bpann_enn(work_dir: str) -> tuple[ENN, dict]:
...
def load_existing(work_dir: str) -> int:
...
def _times(model: ENN, x_query: np.ndarray) -> float:
...
def enn_stress(
    *,
    ...
    x_query = make_points(cfg.query_n, num_dim=cfg.num_dim, seed=cfg.query_seed)
    ...
    def emit_checkpoints(current_n: int) -> Iterator[tuple[int, float, float]]:
        ...
        while (
            ...
            # schedule_flush inside the add loop, matching the
    ...
    try:
        n = 0
        if cfg.batch_size <= 1:
            row_iter = iter_observations(num_obs, num_dim=cfg.num_dim, seed=cfg.seed)
            while True:
                try:
...
def stress_width(num_obs: int) -> int:
...
def format_row(n: int, query_s: float, segment_s: float, *, n_width: int) -> str:
...
def run_stress(
    *,
    ...
    t0 = time.perf_counter()
    model, meta = bpann_enn(work_dir)
    num_dim = int(meta["num_dim"])
    x_query = make_points2(
        cfg.num_samples,
        num_dim=num_dim,
        seed=cfg.seed,
        low=cfg.x_low,
    ...
    function_seeds = function_sample(cfg.seed)
...
def format_header2(*, result: SampleStressResult, work_dir: str) -> str:
    return (
...
def format_summary(result: SampleStressResult) -> str:
    return (
...
def _drawindices(x_test: np.ndarray, draws: np.ndarray) -> np.ndarray:
...
def argmin_rms(x_test: np.ndarray, draws: np.ndarray) -> float:
    ...
    """
    x_arr = np.asarray(x_test, dtype=float)
    i_star = _drawindices(x_test, draws)
    eps = x_arr[i_star] - DRAW_F_CENTER
    return float(np.sqrt(np.mean(np.sum(eps * eps, axis=-1))))


def argmin_rate(x_test: np.ndarray, draws: np.ndarray) -> float:
...
def average_draws(y: np.ndarray, draws: np.ndarray) -> float:
...
class DrawStressConfig:
    ...
    num_candidates: int = DEFAULT_DRAW_NUM_FIT_CANDIDATES
    num_samples: int = DEFAULT_DRAW_NUM_FIT_SAMPLES
...
class DrawStressResult:
    ...
    posterior_draw: DrawMethodResult


def run_stress2(config: DrawStressConfig) -> DrawStressResult:
    ...
    x, y = make_observations2(config.num_obs, num_dim=config.num_dim, rng=data_rng)
    x_test, y_test = make_observations2(
        config.num_test, num_dim=config.num_dim, rng=data_rng
    )
    model = ENN(x, y, scale_x=False)
    ...
    function_seeds = function_sample(config.seed + 4, config.num_draws)
    fn_draws, _idx = model.posterior_draw(
        x_test,
        fitted,
        function_seeds=function_seeds,
        flags=DRAW_FLAGS_NO_OBS,
    )
    avg_lik_fn = average_draws(y_test, fn_draws)
    fn_argmin_rms = argmin_rms(x_test, fn_draws)
    fn_argmin_hit_rate = argmin_rate(x_test, fn_draws)
    eval_fn_s = time.perf_counter() - t2
    function_result = DrawMethodResult(
        method="posterior_draw",
        avg_likelihood=avg_lik_fn,
        argmin_rms=fn_argmin_rms,
        argmin_rate=fn_argmin_hit_rate,
    ...
    return DrawStressResult(
        ...
        epistemic_scale=float(fitted.epistemic_scale),
        aleatoric_scale=float(fitted.aleatoric_scale),
        fit_s=fit_s,
        posterior=posterior_result,
        posterior_draw=function_result,
    )
...
class DrawStressAggregate:
    ...
    num_samples: int
    num_draws: int
    epistemic_scale: MeanSE
    aleatoric_scale: MeanSE
    fit_s: MeanSE
    posterior: DrawMethodAggregate
    posterior_draw: DrawMethodAggregate
...
def _aggregatemethod(
    ...
    return DrawMethodAggregate(
        ...
        argmin_rate=mean_se([r.argmin_rate for r in results]),
        eval_s=mean_se([r.eval_s for r in results]),
    )


def run_seeds(config: DrawStressConfig, *, num_seeds: int) -> DrawStressAggregate:
    """Run ``run_stress2`` for ``seed .. seed+num_seeds-1`` and aggregate metrics."""
    if num_seeds < 1:
        raise ValueError("num_seeds must be >= 1")
    results = [
        run_stress2(
    ...
    return DrawStressAggregate(
        ...
        num_samples=config.num_samples,
        num_draws=config.num_draws,
        epistemic_scale=mean_se([r.epistemic_scale for r in results]),
        aleatoric_scale=mean_se([r.aleatoric_scale for r in results]),
        fit_s=mean_se([r.fit_s for r in results]),
        posterior=_aggregatemethod("posterior", [r.posterior for r in results]),
        posterior_draw=_aggregatemethod(
            "posterior_draw", [r.posterior_draw for r in results]
        ),
    )


def format_header3(result: DrawStressResult) -> str:
    return (
        f"num_dim={result.num_dim} num_obs={result.num_obs} "
        f"num_test={result.num_test} seed={result.seed} k={result.k} "
        f"num_draws={result.num_draws} "
        f"epistemic_scale={result.epistemic_scale:.6g} "
        f"aleatoric_scale={result.aleatoric_scale:.6g} "
        f"fit_s={result.fit_s:{DURATION_S_FMT}}"
    )


def format_summary2(method: DrawMethodResult) -> str:
    return (
        f"{method.method} avg_likelihood={method.avg_likelihood:.6g} "
        f"argmin_rms={method.argmin_rms:.6g} "
        f"argmin_rate={method.argmin_rate:0.4f} "
        f"eval_s={method.eval_s:{DURATION_S_FMT}}"
    )


def format_aggregate(result: DrawStressAggregate) -> str:
    return (
        f"num_dim={result.num_dim} num_obs={result.num_obs} "
        f"num_test={result.num_test} seed={result.seed} "
        f"num_seeds={result.num_seeds} k={result.k} "
        f"num_draws={result.num_draws} "
        f"epistemic_scale={format_se(result.epistemic_scale)} "
        f"aleatoric_scale={format_se(result.aleatoric_scale)} "
        f"fit_s={format_se(result.fit_s, fmt=DURATION_S_FMT)}"
    )


def format_aggregate2(method: DrawMethodAggregate) -> str:
    return (
        f"{method.method} "
        f"avg_likelihood={format_se(method.avg_likelihood)} "
        f"argmin_rms={format_se(method.argmin_rms)} "
        f"argmin_rate={format_se(method.argmin_rate, fmt='0.4f')} "
        f"eval_s={format_se(method.eval_s, fmt=DURATION_S_FMT)}"
    )
...
def enn_stops(num_obs: int, num_ask: int) -> tuple[int, ...]:
    """Return NUM_ASK exponentially spaced cumulative observation stops in [1, NUM_OBS].

    Example: ``enn_stops(30, 4) == (1, 3, 10, 30)``.
    """
    if num_obs < 1:
        raise ValueError("num_obs must be >= 1")
    if num_ask < 1:
...
def enn_config(
    *,
    ...
    enn_kwargs: dict[str, object] = {
        "k": TURBO_ENN_K,
        "fit": ENNFitConfig(num_samples=TURBO_ENN_NUM_FIT_SAMPLES),
        "index_driver": index_driver,
    }
...
def enn_row(n: int, iter_s: float, ask_s: float, tell_s: float, *, n_width: int) -> str:
    return (
...
def enn_stress2(
    *,
    ...
    if num_dim < 1:
        raise ValueError("num_dim must be >= 1")
    stops = enn_stops(num_obs, num_ask)
    if seed_chunk is None:
        seed_chunk = enn_chunk(index_driver)
    if tell_all:
        seed_chunk = 1
    if seed_chunk < 1:
...
def proposal_ns(max_n: int) -> tuple[int, ...]:
...
def _timedround(opt, objective) -> tuple[float, float]:
...
def enn_proposal(
    ...
    *,
    ...
    for _ in range(warmup):
        _timedround(opt, objective)

    ask_times: list[float] = []
    tell_times: list[float] = []
    for _ in range(num_probes):
        ask_s, tell_s = _timedround(opt, objective)
...
def run_stress3(
    ...
    *,
    ...
    for n in grid:
        ...
        ask_mean, tell_mean, proposal_mean = enn_proposal(
            opt,
            ackley,
            warmup=warmup,
...
def enn(
    ...
    if ennx_config is not None:
        from ennx._rust import set_path

        set_path(ennx_config)
    if num_obs < 1:
    ...
    driver = parse_driver2(index_type)
    num_obs_existing = load_existing(work_dir) if work_dir is not None else 0
    click.echo(
        format_header(
            num_dim=num_dim,
            num_obs=num_obs,
    ...
    n_width = stress_width(num_obs)
    batch_size = ENN_ADD_STRESS_BATCH_SIZE if batch else 1
    for n, query_s, segment_s in enn_stress(
        index_driver=driver,
        num_obs=num_obs,
        ...
        click.echo(format_row(n, query_s, segment_s, n_width=n_width))
...
def sample(work_dir: str, num_samples: int, seed: int) -> None:
    ...
    if not result.all_finite:
        raise click.ClickException("posterior_draw returned non-finite values")
    click.echo(format_header2(result=result, work_dir=work_dir))
...
@cli.command(
    ...
    params=[
        ...
        click.Option(
            ["--num-fit-candidates", "num_candidates"],
            type=int,
...
def draw(
    ...
    if num_candidates < 1:
        raise click.ClickException("num_candidates must be >= 1")
    if num_samples < 1:
        raise click.ClickException("num_samples must be >= 1")
    if num_draws < 2:
    ...
    try:
        agg = run_seeds(config, num_seeds=num_seeds)
    except ValueError as exc:
        raise click.ClickException(str(exc)) from exc
    click.echo(format_aggregate(agg))
    click.echo(format_aggregate2(agg.posterior))
    click.echo(format_aggregate2(agg.posterior_draw))
