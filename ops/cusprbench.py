...
from ennx.search import Optimizer, Parameter
...
def main() -> None:
    ...
    for round_index in range(18):
        ...
        trial = search.ask(seeds, min(8, search.history_len), acquisition="thompson")
        elapsed.append((time.perf_counter() - started) * 1_000)
        search.tell(trial, float(round_index + 1))
    measured = elapsed[8:]
