...
optimizer = create_optimizer(
    bounds=bounds,
    config=turbo_zero(candidate_rv=CandidateRV.UNIFORM),
    rng=rng,
)
x_arms = optimizer.ask(num_arms=1)
optimizer.tell(x_arms, objective(x_arms))
""",
    "examples/demombe.ipynb": """
import numpy as np
import torch
from ennx import create_optimizer
from ennx.benchmarks import DoubleAckley
from ennx.turbo.config import MorboTRConfig, MultiObjectiveConfig, turbo_zero
...
def _runsmoke(notebook_path: str) -> None:
...
def _executekernel(nb, repo_root: Path, kernel_manager) -> None:
...
def run_notebook(notebook_path: str) -> None:
    ...
    try:
        _executekernel(nb, repo_root, km)
    finally:
...
def test_demoennnotebook():
    run_notebook("examples/demo_enn.ipynb")


def test_001():
    run_notebook("examples/demote.ipynb")


def test_002():
    run_notebook("examples/demombe.ipynb")


def test_colab():
    repo_root = Path(__file__).resolve().parent.parent
    path = repo_root / "examples/colabjaxcux.ipynb"
...
def test_bf16():
    repo_root = Path(__file__).resolve().parent.parent
    path = repo_root / "examples/colabcuoxdev.ipynb"
    notebook = json.loads(path.read_text(encoding="utf-8"))
