...
def test_001():
    ...
    seeds = [1, 2, 3]
    draws, idx = model.posterior_draw(x_test, params, function_seeds=seeds, flags=flags)
    draws_cond, idx_cond = model.conditional_draw(
...
def test_002():
    ...
    params = ENNParams(k_neighbors=1, epistemic_scale=1.0, aleatoric_scale=1.0)
    flags = PosteriorFlags(exclude_nearest=False, observation_noise=True)
    seeds = [123, 124]
    draws1, idx1 = model.conditional_draw(
        x_whatif,
        y_whatif,
        x_test,
        params=params,
    ...
    draws2, idx2 = model.conditional_draw(
        x_whatif,
        y_whatif,
        x_test,
        params=params,
...
def test_003():
    ...
    params = ENNParams(k_neighbors=1, epistemic_scale=1.0, aleatoric_scale=1.0)
    seeds = [7]
    draw_base = model.posterior_draw(x_test, params, function_seeds=seeds)[0][0, 0, 0]
    draw_incl = model.conditional_draw(
        x_whatif,
        y_whatif,
        x_test,
        params=params,
    ...
    draw_excl = model.conditional_draw(
        x_whatif,
        y_whatif,
        x_test,
        params=params,
...
def test_004():
    ...
    enn_a = ENN(train_x, train_y, train_yvar=None)
    draws_a, idx_a = enn_a.conditional_draw(
        x_whatif,
        y_whatif,
        x_test,
        params=params,
    ...
    draws_b, idx_b = enn_b.posterior_draw(
