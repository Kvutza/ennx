...
from ennx.quantization import quantize_e2m1, quantize_int4
...
def test_packodd():
    ...
    np.testing.assert_array_equal(quantize_e2m1(x), np.array([0x20, 0x04]))
...
def test_rustparity():
    ...
    np.testing.assert_array_equal(
        quantize_e2m1(x),
        np.asarray(rust.quantize_e2m1(x), dtype=np.uint8),
    )
