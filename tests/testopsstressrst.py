...
def test_loadnumobsexisting(tmp_path):
    from ops.stress import load_existing

    assert load_existing(str(tmp_path)) == 0
    ...
    assert load_existing(str(tmp_path)) == 42

    (tmp_path / "num_obs.bin").write_bytes(struct.pack("<Q", 7))
    assert load_existing(str(tmp_path)) == 7
...
def test_002(tmp_path, index_type, subdir):
