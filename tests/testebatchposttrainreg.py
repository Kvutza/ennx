...
def _userdata(rng: np.random.Generator):
...
def test_001():
    ...
    train_x, train_y = _userdata(rng)
    model = ENN(train_x, train_y, train_yvar=None)
    params = ENNParams(
        10,
        epistemic_scale=80.0,
        aleatoric_scale=0.0,
    )
...
def test_003():
    ...
    paramss = [
        ENNParams(3, epistemic_scale=0.5, aleatoric_scale=0.1),
        ENNParams(3, epistemic_scale=2.0, aleatoric_scale=0.0),
    ]
