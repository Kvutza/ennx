...
def test_002():
    ...
    model, params = mk_enn(
        ...
        yvar_obs=yvar_obs,
        rng=rng,
        fit=ENNFitConfig(num_samples=4, num_candidates=6),
    )
...
def test_003():
    ...
    assert params_default.epistemic_scale == 1.0
    assert params_default.aleatoric_scale == 0.0
    assert params_fitted.epistemic_scale > 0.0
    assert (
        abs(params_fitted.epistemic_scale - 1.0) > 1e-12
        or abs(params_fitted.aleatoric_scale - 0.0) > 1e-12
    )
