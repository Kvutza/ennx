...
def _makedata2(*, rng, n: int, d: int, noise_std: float):
...
def test_001():
    ...
    train_x, train_y, train_yvar = _makedata2(rng=rng, n=20, d=3, noise_std=0.1)
    model_base = ENN(train_x, train_y, train_yvar)
    model_scaled = ENN(train_x, train_y * 100.0, train_yvar * 10000.0)
...
def test_002():
    ...
    train_x, train_y, train_yvar = _makedata2(rng=rng, n=20, d=3, noise_std=0.1)
    model_base = ENN(train_x, train_y, train_yvar)
    model_shifted = ENN(train_x, train_y + 1000.0, train_yvar)
    x_test, params = rng.standard_normal((10, 3)), _params(5)
    post_base = model_base.posterior(x_test, params=params)
...
def test_003():
    ...
    params = ENNParams(k_neighbors=7, epistemic_scale=1.0, aleatoric_scale=0.0)
    model = ENN(train_x, train_y, 0.1 * np.ones_like(train_y), scale_x=True)
    model_scaled = ENN(
