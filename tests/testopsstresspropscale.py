...
def test_001():
    assert proposal_ns(1000) == (10, 30, 100, 300, 1000)
    assert proposal_ns(TURBO_ENN_NUM_INIT) == (10,)
    assert proposal_ns(100000)[-1] == 100000
    with pytest.raises(ValueError, match="max_n must be >="):
        proposal_ns(TURBO_ENN_NUM_INIT - 1)
    with pytest.raises(ValueError, match="max_n must be >="):
        proposal_ns(0)
...
def test_005(monkeypatch):
    ...
    monkeypatch.setattr("ops.stress.time.perf_counter", fake_counter)
...
def test_006():
    with pytest.raises(ValueError, match="num_probes must be >="):
        enn_proposal(object(), lambda x: np.array([0.0]), num_probes=0)


def test_007():
    assert (
        format_header4(num_dim=10, max_n=1000, num_probes=30, index_type="flat")
    ...
    assert (
        format_row2(100, 0.1234, 0.01, 0.1334, n_width=4) == " 100 0.1234 0.0100 0.1334"
    )
...
def test_009(monkeypatch):
    ...
    monkeypatch.setattr("ops.stress.run_stress3", fake_run)
...
def test_010(args, fragment):
    cli_rejects(args, fragment)


def test_011(tmp_path, monkeypatch):
    from ops.stress import ProposalScaleResult, cli
