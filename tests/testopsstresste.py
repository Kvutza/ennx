...
from tests.opsstressclihelp import cli_rejects
from tests.opsstresstehelp import (
    RecordingObjective,
    RecordingTurboOpt,
    collect_sizes,
    unit_bounds,
...
def test_001():
    assert enn_stops(30, 4) == (1, 3, 10, 30)
...
def test_003(num_obs, num_ask, fragment):
    with pytest.raises(ValueError, match=fragment):
        enn_stops(num_obs, num_ask)


def test_004():
    cfg = enn_config(index_driver=ENNIndexDriver.FLAT)
    ...
    assert cfg.surrogate.num_samples == TURBO_ENN_NUM_FIT_SAMPLES
...
def test_005():
    with pytest.raises(ValueError, match="bpann_disk requires work_dir"):
        enn_config(index_driver=ENNIndexDriver.BPANN_DISK)
...
def test_008():
    assert (
        enn_header(num_dim=10, num_obs=30, num_ask=4, index_type="flat")
    ...
    assert "tell_all" not in enn_header(
        num_dim=2, num_obs=10, num_ask=3, index_type="flat", tell_all=False
    )
    assert enn_row(1, 0.1234, 0.01, 0.02, n_width=2) == " 1 0.1234 0.0100 0.0200"
...
def test_011(num_obs, num_ask):
    stops = enn_stops(num_obs, num_ask)
    gaps = [stops[0]] + [stops[i] - stops[i - 1] for i in range(1, len(stops))]
    assert sum(gaps) == num_obs
    assert all(g >= 1 for g in gaps)

    tell_sizes = collect_sizes(num_obs=num_obs, num_ask=num_ask, tell_all=False)
    assert tell_sizes == gaps
...
def test_013():
    ...
    tell_sizes = collect_sizes(num_obs=5, num_ask=2, tell_all=True, seed_chunk=0)
...
def test_014():
    ...
    num_obs = num_ask = 4
    without = collect_sizes(num_obs=num_obs, num_ask=num_ask, tell_all=False)
    with_flag = collect_sizes(num_obs=num_obs, num_ask=num_ask, tell_all=True)
...
def test_015(monkeypatch):
    ...
    monkeypatch.setattr("ops.stress.enn_stress2", fake_run)
...
def test_017(tmp_path, monkeypatch):
    from ops.stress import TurboEnnRoundResult, cli
...
def test_turboennclirejects(args, fragment):
    cli_rejects(args, fragment)
...
def test_019():
    ...
    assert enn_chunk(ENNIndexDriver.FLAT) == TURBO_ENN_SEED_CHUNK_FLAT
    assert enn_chunk(ENNIndexDriver.BPANN_DISK) == TURBO_ENN_SEED_CHUNK
    sig = inspect.signature(enn_stress2)
    assert sig.parameters["seed_chunk"].default is None


def _enncount(gap: int, chunk: int) -> int:
    ...
    enn_n(opt, ackley, bounds, gap, rng=rng, chunk=chunk)
...
def test_020():
    ...
    small = _enncount(gap, small_chunk)
    large = _enncount(gap, TURBO_ENN_SEED_CHUNK_FLAT)
...
def test_021():
    ...
    flat_tells = _enncount(gap, TURBO_ENN_SEED_CHUNK_FLAT)
    disk_tells = _enncount(gap, TURBO_ENN_SEED_CHUNK)
    assert flat_tells == 1
