...
fn kda_row() {
    ...
    let round = state
        ...
        .unwrap();
    state.bind_metal(&round, &mut executor).unwrap();
}
...
fn run_kda(executor: &mut KdaMoeMetalExecutor) -> Vec<u16> {
    ...
    executor
        .attention_norm(&[0x3c00; 4], 1, 16_384, 1.0e-6)
        .unwrap();
...
fn row_forward() {
    ...
    let mut state = ResidentBoState::new(
        &packed,
        0.0,
        vec![Parameter::new(0, 48, 8, 1.0, 1.0, 1.0).unwrap()],
        2,
        ComputeDevice::Metal,
        ForwardProgram::kda().unwrap(),
    )
    ...
    let round = state
        ...
        .unwrap();
    let mut materialized = kda_executor(
        &request,
        control,
        &packed,
        &scales,
...
fn kda_state() {
    ...
    executor
        .kda_inputs(&vector, &vector, &[0x4000; 4], &[0.0; 4], &[0.5])
        .unwrap();
...
fn single_resident() {
    ...
    model
        .attach_head(linear(4, 8, 8), linear(4, 1, 8), 1.0)
        .unwrap();
    model.reset_state();
...
fn decode_layer() {
    ...
    let mut allocate = |input_width, output_width, bits, group_size| {
        ...
        scale_offset += descriptor.row_groups() * output_width;
    ...
    for _ in 0..24 {
        let mut layer = KdaMoeMetalExecutor::new_arena(
            &request,
            control,
            KdaMoeMetalKdaVectors {
                decay: &[0.0; 8],
