...
Example:
  python scripts/cmpxvers.py
  python scripts/cmpxvers.py --quick
  python scripts/cmpxvers.py worker --optimizer turbo_enn --problem ackley_30d --version current
"""
...
from scripts.cmpxverscore import (  # noqa: E402
    OPTIMIZER_NAMES,
    PROBLEMS,
    BenchmarkResult,
    apply_overrides3,
...
def _pythonpathsrc(env: dict[str, str], src: str) -> None:
...
def _pythonpathsrc2(env: dict[str, str], src: str) -> None:
...
def _envversion(version_label: str) -> dict[str, str]:
    env = os.environ.copy()
    src = str(ROOT / "src")
    if version_label == CURRENT_LABEL:
        _pythonpathsrc(env, src)
    else:
        _pythonpathsrc2(env, src)
    return env
...
def _workermain(args: argparse.Namespace) -> int:
    problems = apply_overrides3(PROBLEMS) if args.quick else PROBLEMS
...
def _formatrow(result: BenchmarkResult) -> str:
    return (
...
def _main(args: argparse.Namespace) -> int:
    ...
    out_path = ROOT / "cmpxvers_report.json"
    report = {
...
def _buildparser2() -> argparse.ArgumentParser:
