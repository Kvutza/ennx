...
FIXTURE_OBJECTIVES: dict[str, Callable[[np.ndarray], np.ndarray]] = {
    "sphere_centered_0.5": sphere_objective,
    "separable_unimodal_two_metric": separable_objective,
}
...
def name_prefix(name: str) -> str:
    m = re.fullmatch(r"(.+seed)[0-9]+", name)
    return m.group(1) if m else name


def entry_name(name: str) -> FixtureGeneratorEntry:
...
def subdir_entry(entry: FixtureGeneratorEntry) -> str:
...
EXPECTED_OPTIMIZER_FIXTURE_NAMES = catalog_names()
