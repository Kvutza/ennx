    ...
    def ask(
        ...
        *,
        ...
        if not isinstance(model, PyENN):
            raise TypeError(f"Expected ENN, got {type(model)}")

        rust_warm_start = None
        if params_warm_start is not None:
            rust_warm_start = RustENNParams(
                params_warm_start.k_neighbors,
                params_warm_start.epistemic_scale,
                params_warm_start.aleatoric_scale,
            )
        ...
        return PyENNParams(
            k_neighbors=rust_result.k_neighbors,
            epistemic_scale=rust_result.epistemic_scale,
            aleatoric_scale=rust_result.aleatoric_scale,
        )
