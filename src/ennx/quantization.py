...
def quantize_e2m1(arr: np.ndarray, scale: float = 1.0) -> np.ndarray:
    """Quantize float values to nearest FP4 (E2M1) representation and pack into uint8 bytes."""
    if _rust is not None:
        return np.asarray(_rust.quantize_e2m1(arr, scale), dtype=np.uint8)
    scaled = arr.ravel() / scale
    # Find nearest index in FP4 LUT
    diffs = np.abs(scaled[:, None] - FP4_LUT[None, :])
